...
bool UCardEffect_RemoveCardFromPile::ResolveCardEffect(AActor* TargetActor)
{
	...
	if (!TargetComponent->IsChildOf(UPileComponent::StaticClass())) return false;

	// PileComponent를 가져옴
	UPileComponent* TargetPileComponent = Cast<UPileComponent>(CardPlayer->GetComponentByClass(TargetComponent));
	if (!TargetPileComponent) return false; // PileComponent를 찾지 못하면 실패

	// PileComponent가 IInterface_Pile 인터페이스를 구현하는지 확인
	if (!TargetPileComponent->GetClass()->ImplementsInterface(UInterface_Pile::StaticClass())) return false; // 인터페이스를 구현하지 않으면 실패
